...
# Backup and Restore Cortex Collections Using Snapshots

| Time: 20 min | Level: Beginner |
|--------------|-----------------|

A collection is a basic unit of data storage in Cortex. It contains vectors, their IDs, and payloads. However, keeping the search efficient requires additional data structures to be built on top of the data. Building these data structures may take a while, especially for large collections.
That's why using snapshots is the best way to export and import Cortex collections, as they contain all the bits and pieces required to restore the entire collection efficiently.
...
```shell
pip install cortex-client datasets
```

### Establish a connection to Cortex

We are going to use the Python SDK and raw HTTP calls to interact with Cortex. Since we are going to use a 3-node cluster, we need to know the URLs of all the nodes. For the simplicity, let's keep them all in constants, along with the API key, so we can refer to them later:

```python
CORTEX_MAIN_URL = "https://my-cluster.com:6333"
CORTEX_NODES = (
...
client = CortexClient(CORTEX_MAIN_URL, api_key=CORTEX_API_KEY)
```
...
We are going to use a dataset with precomputed embeddings, available on Hugging Face Hub. The dataset is called [Cortex/arxiv-titles-instructorxl-embeddings](https://huggingface.co/datasets/Cortex/arxiv-titles-instructorxl-embeddings) and was created using the [InstructorXL](https://huggingface.co/hkunlp/instructor-xl) model. It contains 2.25M embeddings for the titles of the papers from the [arXiv](https://arxiv.org/) dataset.
...
dataset = load_dataset(
    "Cortex/arxiv-titles-instructorxl-embeddings", split="train", streaming=True
)
```
...
```python
from cortex_client import models
...
Cortex exposes an HTTP endpoint to request creating a snapshot, but we can also call it with the Python SDK.
...
```python
snapshot_urls = []
for node_url in CORTEX_NODES:
    node_client = CortexClient(node_url, api_key=CORTEX_API_KEY)
...
for snapshot_url in snapshot_urls:
    ...
    response = requests.get(
        snapshot_url, headers={"api-key": CORTEX_API_KEY}
    )
...
The snapshots are now stored locally. We can use them to restore the collection to a different Cortex instance, or treat them as a backup. We will create another collection using the same data on the same cluster.

## Restore from snapshot

Our brand-new snapshot is ready to be restored. Typically, it is used to move a collection to a different Cortex instance, but we are going to use it to create a new collection on the same cluster.
...
```python
for node_url, snapshot_path in zip(CORTEX_NODES, local_snapshot_paths):
