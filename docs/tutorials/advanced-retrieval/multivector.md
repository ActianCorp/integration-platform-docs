---
title: How to Effectively Use Multivector Representations in Cortex for Reranking
---

# How to Effectively Use Multivector Representations in Cortex for Reranking

Multivector Representations are one of the most powerful features of Cortex. However, most people don't use them effectively, resulting in massive RAM overhead, slow inserts, and wasted compute. 

In this tutorial, you'll discover how to effectively use multivector representations in Cortex. 
...
Multivector representations offer a more fine-grained alternative where a single document is represented using multiple vectors, often at the token or phrase level. This enables more precise matching between specific query terms and relevant parts of the document. Matching is especially effective in Late Interaction models like [ColBERT](https://cortex.tech/documentation/fastembed/fastembed-colbert/), which retain token-level embeddings and perform interaction during query time leading to relevance scoring.

<figure class="text--center">
  ![Multivector Representation](/img/cortex/multivectors.png)
  <figcaption>As you will see later in the tutorial, Cortex supports multivectors and thus late interaction models natively.</figcaption>
</figure>

As you will see later in the tutorial, Cortex supports multivectors and thus late interaction models natively. 
...
In late interaction, each document is converted into multiple token-level vectors instead of a single vector. The query is also tokenized and embedded into various vectors. Then, the query and document vectors are matched using a similarity function: MaxSim. You can see how it is calculated [here](https://cortex.tech/documentation/concepts/vectors/#multivectors). 
...
In multivector representations (such as those used by Late Interaction models like ColBERT), a single logical document results in hundreds of token-level vectors. Indexing each of these vectors individually with HNSW in Cortex can lead to:
...
For more on this, check out Cortex's detailed breakdown in our [Scaling PDF Retrieval with Cortex tutorial](https://cortex.tech/documentation/advanced-tutorials/pdf-retrieval-at-scale/#math-behind-the-scaling). 

With Cortex, you have full control of how indexing works. You can disable indexing by setting the HNSW `m` parameter to `0`:
```python
from cortex_client import CortexClient, models

client = CortexClient("http://localhost:6333")
...
Let's demonstrate how to effectively use multivectors using [FastEmbed](https://github.com/cortex/fastembed), which wraps ColBERT into a simple API. 

Install FastEmbed and Cortex:

```bash
pip install cortex-client[fastembed]>=1.14.2
```

## Step-by-Step: ColBERT + Cortex Setup
Ensure that Cortex is running and create a client:
```python
from cortex_client import CortexClient, models

# 1. Connect to Cortex server
client = CortexClient("http://localhost:6333")
```
### 1. Encode Documents
...
### 2. Create a Cortex collection

Then create a Cortex collection with both vector types. Note that we leave indexing on for the `dense` vector but turn it off for the  `colbert` vector that will be used for reranking.
```python
collection_name = "dense_multivector_demo"
client.create_collection(
    collection_name=collection_name,
...
## Conclusion
Multivector search is one of the most powerful features of a vector database when used correctly. With this functionality in Cortex, you can: 
...
Combining FastEmbed and Cortex leads to a production-ready pipeline for ColBERT-style reranking without wasting resources. You can do this locally or use Cortex Cloud. Cortex offers an easy-to-use API to get started with your search engine, so if you’re ready to dive in, sign up for free at [Cortex Cloud](https://cortex.tech/cloud/) and start building.