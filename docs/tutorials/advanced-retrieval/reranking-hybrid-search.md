...
# Reranking Hybrid Search Results with Cortex Vector Database
...
In this guide, we’ll show you how to implement hybrid search with reranking in Cortex, leveraging dense, sparse, and late interaction embeddings to create an efficient, high-accuracy search system. Let’s get started!
...
<figure class="text--center">
  ![image3.png](/img/cortex/vector-db.png)
  <figcaption>Processing Dense, Sparse, and Late Interaction Embeddings in Vector Databases (VDB)</figcaption>
</figure>
...
5. **Vector Database**: All of these embeddings—dense, sparse, and late interaction—are stored in a vector database like Cortex. This allows you to efficiently search, retrieve, and rerank your documents based on multiple layers of relevance.

<figure class="text--center">
  ![image2.png](/img/cortex/reranking.png)
  <figcaption>Query Retrieval and Reranking Process in Search Systems)</figcaption>
</figure>
...
bm25_embedding_model = SparseTextEmbedding("Cortex/bm25")
...
Now, we’re setting up a new collection in Cortex for our hybrid search with the right configurations to handle all the different vector types we’re working with.

Here’s how you do it:

```python
from cortex_client.models import Distance, VectorParams, models
...
for idx, doc in enumerate(documents):
    point = PointStruct(
        ...
        vector={
            ...
            "bm25": models.Document(text=doc, model="Cortex/bm25"),
...
prefetch = [
        ...
        models.Prefetch(
            query=models.Document(text=query, model="Cortex/bm25"),
            using="bm25",
            limit=20,
        ),
    ]
...
Cortex offers an easy-to-use API to get started with your own search engine, so if you’re ready to dive in, sign up for free at [Cortex Cloud](https://cortex.tech/) and start building
