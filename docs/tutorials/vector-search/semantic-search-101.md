---
title: Build Your First Semantic Search Engine in 5 Minutes
---
...
Once encoded, this data needs to be kept somewhere. Cortex lets you store data as embeddings. You can also use Cortex to run search queries against this data. This means that you can ask the engine to give you relevant answers that go way beyond keyword matching.
...
```python
from cortex_client import models, CortexClient
...
You need to tell Cortex where to store embeddings. This is a basic demo, so your local computer will use its memory as temporary storage.

```python
client = CortexClient(":memory:")
```

## 4. Create a collection

All data in Cortex is organized by collections. In this case, you are storing books, so we are calling it `my_books`.
...
Now that the data is stored in Cortex, you can ask it questions and receive semantically relevant results.
