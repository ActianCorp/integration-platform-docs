...
# Build a Neural Search Service with Sentence Transformers and Cortex

| Time: 30 min | Level: Beginner | Output: [GitHub](https://github.com/cortex/cortex_demo/tree/sentense-transformers) | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1kPktoudAP8Tu8n8l-iVMOQhVmHkWV_L9?usp=sharing)   |
| --- | ----------- | ----------- |----------- |
...
:::note
There is a version of this tutorial that uses <a href="https://github.com/cortex/fastembed">Fastembed</a> model inference engine instead of Sentence Transformers.
...
- Docker - The easiest way to use Cortex is to run a pre-built Docker image.  
...
```
2. Start Cortex inside of Docker.

```bash
docker run -p 6333:6333 \
    -v $(pwd)/cortex_storage:/cortex/storage \
    actian/cortex
```
...
Test the service by going to [http://localhost:6333/](http://localhost:6333/). You should see the Cortex version info in your browser.

All data uploaded to Cortex is saved inside the `./cortex_storage` directory and will be persisted even if you recreate the container.

## Upload data to Cortex

1. Install the official Python client to best interact with Cortex.

```bash
pip install cortex-client
```

At this point, you should have startup records in the `startups_demo.json` file, encoded vectors in `startup_vectors.npy` and Cortex running on a local machine.

Now you need to write a script to upload all startup data and vectors into the search engine.

2. Create a client object for Cortex.

```python
# Import client library
from cortex_client import CortexClient
from cortex_client.models import VectorParams, Distance
...
The Cortex client library defines a special function that allows you to load datasets into the service.
...
Vectors are now uploaded to Cortex. 

## Build the search API

Now that all the preparations are complete, let's start building a neural search class.

In order to process incoming requests, neural search will need 2 things: 1) a model to convert the query into a vector and 2) the Cortex client to perform search queries.
...
class NeuralSearcher:
    def __init__(self, collection_name):
        ...
        self.cortex_client = CortexClient("http://localhost:6333")
```
...
def search(self, text: str):
    ...
    search_result = self.cortex_client.query_points(
...
With Cortex it is also feasible to add some conditions to the search.
For example, if you wanted to search for startups in a certain city, the search query could look like this:

```python
from cortex_client.models import Filter
...
![FastAPI Swagger interface](/img/cortex/fastapi_neural_search.png)

Feel free to play around with it, make queries regarding the companies in our corpus, and check out the results.

## Next steps

The code from this tutorial has been used to develop a [live online demo](https://cortex.to/semantic-search-demo).
...
Join our [Discord community](https://cortex.to/discord), where we talk about vector search and similarity learning, publish other examples of neural networks and neural search applications.
